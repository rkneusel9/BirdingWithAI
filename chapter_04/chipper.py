...
class ImagePanel(wx.Panel):
    def __init__(self, parent, isize, outdir):
    ...
    def save_chip(self, chip):
        ...
        img = Image.fromarray(chip).resize((self.isize,self.isize), resample=Image.BILINEAR)
        img.save(chip_path)
        wx.MessageBox(f"Saved chip to {chip_path}", "Chip Saved", wx.OK | wx.ICON_INFORMATION)
...
class MainFrame(wx.Frame):
    def __init__(self, *args, **kw):
        ...
        self.last_directory = "~"
        self.panel = ImagePanel(self, int(sys.argv[1]), sys.argv[2])

        self.create_menu()
        self.SetSize((900, 675))
        self.SetTitle('Chip Extractor')
        self.Centre()
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) == 1):
        print()
        print("chipper <size> <outdir>")
        print()
        print("  <size> - chip output size (always square)")
